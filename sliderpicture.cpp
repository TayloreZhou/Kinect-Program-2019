#include "sliderpicture.h"

SliderPicture::SliderPicture(QWidget *parent)
    : QWidget(parent)
    , mouse_press(false)
    , mouse_move(false)
    , label_move(true)
    , current_index(0)
    , current_pos_x(0)
    , cate(0)
{
    //************设置屏幕大小*************//
    setWindowFlags(Qt::FramelessWindowHint | Qt::Dialog);
    resize(1280, 720);

    //************设置背景图片*************//
    background_label = new QLabel(this);
    background_label->setPixmap(QPixmap(":/img/bg_bottom"));
    background_label->setGeometry(QRect(0, 0, this->width(), this->height()));

    //************在画布上排布图片*************//
    QPixmap pixmap(QSize(this->width()*6, this->height()*4));
    QPainter painter(&pixmap);
    for(int i = 0; i < 6; i++){
        painter.drawImage(QRect(this->width()*i, 0, this->width(), this->height()),
                          QImage(QString(":/img/info0_%1").arg(i)));
    }
    for(int i = 0; i < 6; i++){
        painter.drawImage(QRect(this->width()*i, this->height(), this->width(), this->height()),
                          QImage(QString(":/img/info1_%1").arg(i)));
    }
    for(int i = 0; i < 6; i++){
        painter.drawImage(QRect(this->width()*i, this->height()*2, this->width(), this->height()),
                          QImage(QString(":/img/info2_%1").arg(i)));
    }
    for(int i = 0; i < 6; i++){
        painter.drawImage(QRect(this->width()*i, this->height()*3, this->width(), this->height()),
                          QImage(QString(":/img/info3_%1").arg(i)));
    }

    /////////
    total_label = new QLabel(this);
    total_label->resize(pixmap.size());
    total_label->setPixmap(pixmap);
    total_label->move(0, 0);
    /////////

    //************button的排布和时间的connect*************//
    for(int index = 0; index < 6; index++) {
        PixLabel *label = new PixLabel(this);
        label->resize(QSize(100, 100));
        label->setPixmap(QPixmap(QString(":/img/c1i%1").arg(index)));
//        if (index == 0) {
//            label->setText(tr("function"));
//        }
//        else if (index == 1) {
//            label->setText(tr("clearcookie"));
//        }
//        else if (index == 2) {
//            label->setText(tr("triggerman"));
//        }
//        else {
//            label->setText(tr("booster"));
//        }
        label->move(40+index*200, 570);
        connect(label, SIGNAL(signalLabelPress(PixLabel*)), this, SLOT(changeCurrentPage(PixLabel*)));
        label_array.append(label);
    }
    label_array.first()->setMousePressFlag(true);

//===========上边栏===============//

    for(int index = 0; index < 4; index++) {
        PixLabel *cate = new PixLabel(this);
        cate->resize(QSize(100, 100));


        QSize picSize(200,200);
        QPixmap myPic(QString(":/img/cate%1").arg(index));
        QPixmap scaledPic = myPic.scaled(picSize, Qt::KeepAspectRatio);
//        myPic->setAlignment(Qt::AlignCenter);
//        myPic.scaled(200, 200, Qt::KeepAspectRatio, Qt::SmoothTransformation);
//        QPixmap scaledPic = myPic.scaled(200,200,Qt::IgnoreAspectRatio,Qt::FastTransformation);
        cate->setPixmap(scaledPic);
//        QPixmap pic(QPixmap(filename).scaled(20, 20, Qt::IgnoreAspectRatio,
//                                          Qt::FastTransformation));
//        cate->setPixmap(QPixmap(QString(":/img/cate%1").arg(index)).scaled(200,200,Qt::IgnoreAspectRatio,Qt::FastTransformation));


        cate->move(40+index*200, 50);
        connect(cate, SIGNAL(signalLabelPress(PixLabel*)), this, SLOT(changeCategory(PixLabel*)));
        cate_array.append(cate);
    }
	PixLabel *confirm = new PixLabel(this);
	confirm->resize(QSize(100, 100));
	confirm->setText(QString("Confirm"));
	confirm->move(40 + 4 * 200, 50);
	connect(confirm, SIGNAL(signalLabelPress(PixLabel*)), this, SLOT(nextWindow()));
	PixLabel *exit = new PixLabel(this);
	exit->resize(QSize(100, 100));
	exit->setText(QString("Exit"));
	exit->move(40 + 5 * 200, 50);
	connect(exit, SIGNAL(signalLabelPress(PixLabel*)), this, SLOT(exitM()));

    cate_array.first()->setMousePressFlag(true);
    cate_array.first()->setFocus();

//===========上边栏===============//

    close_button = new PushButton(this);
    close_button->setPicName(":/img/close");
    close_button->move(this->width()-close_button->width(), 0);
    close_button->setToolTip(tr("close"));
    connect(close_button, SIGNAL(clicked()), this, SLOT(close()));
}

SliderPicture::~SliderPicture()
{

}

void SliderPicture::mousePressEvent(QMouseEvent *event)
{
    if(event->button() == Qt::LeftButton) {

//     点击位置距离上边界40px时，可以移动整个界面
//        m_mouseSrcPos = event->pos();
//        if(m_mouseSrcPos.y() <= 40) {
//            mouse_move = true;
//        }
//        else {
//            current_pos_x = total_label->x();
//            mouse_press = true;
//        }
        m_mouseSrcPos = event->pos();
        current_pos_x = total_label->x();

        mouse_press = true;
    }
}


//************[未释放]鼠标拖动来滑动图片*************//
//************在函数内实现*************//
//************！！移动界面*************//
void SliderPicture::mouseMoveEvent(QMouseEvent *event)
{
    int xPos = 0;
    //滑动图片
    if(mouse_press) {
        if(label_move) {
            m_mouseDstPos = event->pos();
            xPos = m_mouseDstPos.x() - m_mouseSrcPos.x();

            setLabelMove(false);
            total_label->move(current_pos_x + xPos, -cate*720);
            setLabelMove(true);

            //DEBUG
//            setLabelMove(false);
//            total_label->move(current_pos_x, current_pos_y + yPos);
//            setLabelMove(true);
        }
    }

    //移动整个界面
    else if(mouse_move) {
        m_mouseDstPos = event->pos();
        this->move(this->pos() + m_mouseDstPos - m_mouseSrcPos);
    }
}


//************[释放后]图片的移动*************//
//************具体移动通过moveCurrentPage()实现*************//
void SliderPicture::mouseReleaseEvent(QMouseEvent *event)
{
    int xpos = 0;
    if (mouse_press) {
        if (label_move) {
            m_mouseDstPos = event->pos();
            xpos = m_mouseDstPos.x() - m_mouseSrcPos.x();

            //如果移动距离大于400px,且不是第一张图片，就切换图片，否则弹回当前图片
            if (xpos > 0) {                     //the plan is:move right
                if(xpos >= 400) {               //mouse gap
                    if(current_index > 0) {     //move right
                        current_index--;
                        moveCurrentPage(false);
                    }
                    else {
                        moveCurrentPage(true);  //move left
                    }
                }
                else {
                     moveCurrentPage(true);     //move left
                }
            }
            else if(xpos<0) {             //the plan is:move right
                if(xpos <= -400)
                {
                    if(current_index < 6-1) {
                        current_index++;
                        moveCurrentPage(true);  //move left
                    }
                    else {
                        moveCurrentPage(false); //move right
                    }
                }
                else {
                    moveCurrentPage(false);     //move right
                }
            }
 /********************************************************************************/
//            else{
//                if(m_mouseDstPos.y()>300&&m_mouseDstPos.y()<420)total_label->move(0,0);
//            }
/********************************************************************************/
        }

        mouse_press = false;
    }
    else if(mouse_move){
        mouse_move = false;
    }
}

//************[释放后]true/false分别向两个方向移动*************//
void SliderPicture::moveCurrentPage(bool direction)
{
    //************初始化mouse_press*************//
    for(int i=0; i<6; i++) {
        if(i != current_index) {
            label_array.at(i)->setMousePressFlag(false);
        }
        else{
            label_array.at(i)->setMousePressFlag(true);
        }
    }

    setLabelMove(false);
    int current_pos_x = total_label->x();    //label position
    int dest_pos_x = -1280 * current_index;
    if(direction) {
        while(current_pos_x > dest_pos_x) {
            total_label->move(current_pos_x-100, -cate*720);
            current_pos_x = total_label->x();
            qApp->processEvents(QEventLoop::AllEvents);
        }
    }
    else {
        while(current_pos_x < dest_pos_x) {
            total_label->move(current_pos_x+100, -cate*720);
            current_pos_x = total_label->x();
            qApp->processEvents(QEventLoop::AllEvents);
        }
    }
    total_label->move(dest_pos_x, -cate*720);
    setLabelMove(true);
}

void SliderPicture::setLabelMove(bool enable)
{
    label_move = enable;
}

//************与label的关联的connect函数实现*************//
void SliderPicture::changeCurrentPage(PixLabel *label)
{
    //************获取被点击的label的index*************//
    int index = 0;
    for(int i = 0; i < 6; i++) {
        if(label != label_array.at(i)) {
            label_array.at(i)->setMousePressFlag(false);
        }
        else {
            index = i;
        }
    }
    if(index < current_index) {
        while(index != current_index) {
            current_index--;
            moveCurrentPage(false);
        }
    }
    else if(index > current_index) {
        while(index != current_index) {
            current_index++;
            moveCurrentPage(true);
        }
    }

}

void SliderPicture::changeCategory(PixLabel *c){
    for(int i = 0; i < 4; i++) {
        if(c != cate_array.at(i)) {
            cate_array.at(i)->setMousePressFlag(false);
        }
        else {
            cate = i;
        }
    }

    label_array.at(0)->setMousePressFlag(true);

    for(int i=1; i<6; i++) {
       label_array.at(i)->setMousePressFlag(false);
    }

    current_index = 0;
    current_pos_x = 0;
    setLabelMove(false);
    total_label->move(0, -cate*720);
    setLabelMove(true);

    switch (cate) {
    case 0:
        for(int i=0;i<label_array.size();i++){
            label_array[i]->setPixmap(QPixmap(QString(":/img/c1i%1").arg(i)));
        }
        break;
    case 1:
        for(int i=0;i<label_array.size();i++){
            label_array[i]->setPixmap(QPixmap(QString(":/img/c2i%1").arg(i)));
        }
        break;
    case 2:
        for(int i=0;i<label_array.size();i++){
            label_array[i]->setPixmap(QPixmap(QString(":/img/c3i%1").arg(i)));
        }
        break;
    case 3:
        for(int i=0;i<label_array.size();i++){
            label_array[i]->setPixmap(QPixmap(QString(":/img/c4i%1").arg(i)));
        }
        break;
    default:
        break;
    }
}

//----------PushButton------------//
PushButton::PushButton(QWidget *parent)
    : QPushButton(parent)
{
    status = NORMAL;
    mouse_press = false;
}

PushButton::~PushButton()
{

}

void PushButton::setPicName(QString pic_name)
{
    this->pic_name = pic_name;
    setFixedSize(QPixmap(pic_name).size());
}

void PushButton::enterEvent(QEvent *)
{
    status = ENTER;
    update();
}

void PushButton::mousePressEvent(QMouseEvent *event)
{
    if(event->button() == Qt::LeftButton) {
        mouse_press = true;
        status = PRESS;
        update();
    }
}

void PushButton::mouseReleaseEvent(QMouseEvent *event)
{
    if(mouse_press  && this->rect().contains(event->pos())) {
        mouse_press = false;
        status = ENTER;
        update();
        emit clicked();
    }
}

void PushButton::leaveEvent(QEvent *)
{
    status = NORMAL;
    update();
}

void PushButton::paintEvent(QPaintEvent *)
{
    QPainter painter(this);
    QPixmap pixmap;
    switch(status) {
        case NORMAL: {
            pixmap.load(pic_name);
            break;
        }
        case ENTER: {
            pixmap.load(pic_name + QString("_hover"));
            break;
        }
        case PRESS: {
            pixmap.load(pic_name + QString("_pressed"));
            break;
        }
        case NOSTATUS: {
            pixmap.load(pic_name);
            break;
        }
        default:
            pixmap.load(pic_name);
    }
    painter.drawPixmap(rect(), pixmap);
}





//--------------PixLabel-------------//
PixLabel::PixLabel(QWidget *parent)
    : QWidget(parent)
{
    initVariable();
    initSetupUi();
}

PixLabel::~PixLabel()
{
    delete m_pLabelIcon;
    delete m_pLabelText;
    delete m_pHLayout;
}

void PixLabel::setPixmap(const QPixmap &pixmap)
{
    m_pLabelIcon->setPixmap(pixmap.scaled(QSize(30, 30), Qt::KeepAspectRatio, Qt::SmoothTransformation));
}

void PixLabel::setText(const QString &text)
{
    m_pLabelText->setText(text);
}

void PixLabel::setMouseEnterFlag(bool flag)
{
    m_mouseEnterFlag = flag;
    this->update();
}

void PixLabel::setMousePressFlag(bool flag)
{
    m_mousePressFlag = flag;
    this->update();
}

void PixLabel::enterEvent(QEvent *)
{
    if(!getMousePressFlag()) {
        setMouseEnterFlag(true);
    }
    this->setCursor(Qt::PointingHandCursor);
}

void PixLabel::leaveEvent(QEvent *)
{
    setMouseEnterFlag(false);
}

void PixLabel::mousePressEvent(QMouseEvent *e)
{
    if(e->button() == Qt::LeftButton) {
        setMousePressFlag(true);
        emit signalLabelPress(this);
    }
}

void PixLabel::paintEvent(QPaintEvent *e)
{
    QPainter painter(this);
    if(getMouseEnterFlag()) {
        paintWidget(50, &painter);
    }
    else if(getMousePressFlag()) {
        paintWidget(80, &painter);
    }
    QWidget::paintEvent(e);
}

void PixLabel::initVariable()
{
    setMouseEnterFlag(false);
    setMousePressFlag(false);
}

void PixLabel::initSetupUi()
{
    createFrame();
    createWidget();
    createLayout();
}

void PixLabel::createFrame()
{
    this->setStyleSheet("QWidget{background:transparent;border:0px;color:white;font-weight:bold;font-size:16px;}");
}

void PixLabel::createWidget()
{
    m_pLabelIcon = new QLabel(this);
    m_pLabelText = new QLabel(this);
}

void PixLabel::createLayout()
{
    m_pHLayout = new QHBoxLayout;
    m_pHLayout->setSpacing(10);
    m_pHLayout->setContentsMargins(QMargins(5, 0, 5, 0));
    m_pHLayout->addWidget(m_pLabelIcon);
    m_pHLayout->addWidget(m_pLabelText);
    m_pHLayout->addStretch();

    this->setLayout(m_pHLayout);
}

void PixLabel::paintWidget(int transparency, QPainter *device)
{
    QPen pen(Qt::NoBrush, 1);
    device->setPen(pen);
    QLinearGradient linear(this->rect().topLeft(), this->rect().bottomLeft());
    linear.setColorAt(0, QColor(255, 255, 255, transparency));

    QBrush brush(linear);
    device->setBrush(brush);
    device->drawRoundedRect(this->rect(), 2, 2);
}

inline bool PixLabel::getMouseEnterFlag()
{
    return m_mouseEnterFlag;
}

inline bool PixLabel::getMousePressFlag()
{
    return m_mousePressFlag;
}

void SliderPicture::exitM()
{
    this->close();
    emit quit();
}
void SliderPicture::exit()
{
    this->close();
}
void SliderPicture::nowWindow()
{
    this->show();
}
void SliderPicture::nextWindow()
{
    this->hide();
	emit kinectWindow(cate, current_index);
}
